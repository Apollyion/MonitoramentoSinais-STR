...
// int getHeartRate()
{
  for (int i = 0; i < setSize; i++)
  {
    ...
    if (checkForBeat(irValue))
    {
      ...
      Serial.println("Entrou if 1 beatavg");
      if (beatsPerMinute < 255 && beatsPerMinute > 20)
      {
        ...
        for (byte x = 0; x < RATE_SIZE; x++)
          beatAvg += rates[x];
        beatAvg /= RATE_SIZE;
        Serial.println("Entrou if 2 beatavg");
      }
      /* Debugging */
      Serial.print("IR=");
...
const char *ssid = "esp32";        // SSID da rede WiFi
const char *password = "pinochet"; // Senha da rede WiFi
// TODO - Alterar para o IP e porta do servidor
const char *serverIP = "192.168.100.133"; // Endereço IP do servidor
...
void setup()
{
  ...
  timerAlarmWrite(timer, 500000000, true); // 3s  5000000
...
void loop()
{
  ...
  */
  ...
  if (agora - lastExecutedWifi >= PERIODO_WIFI)
  {
    ...
    sprintf(packet, "%d %d %d %d", avgBPM, spo2Value, valFall, botaoPressionado);
    send_event(packet);
    Serial.println(packet);
    sprintf(packet, "");
    botaoPressionado = false; // Reiniciar o estado do botao
